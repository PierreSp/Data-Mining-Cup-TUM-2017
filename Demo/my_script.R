...
test_data = read.csv("Demo/test.csv", sep=",")
...
get_best_model <- function(models, methods){
  ...
  return(models[winner_nr])
}


######################################################
# 5. Predict Classes in Test Data
prediction_classes = predict.train(object=get_best_model(models)[[1]], newdata=test_data, na.action=na.pass)
